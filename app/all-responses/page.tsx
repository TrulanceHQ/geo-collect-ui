...
//     // Compare the normalized text of the raw subquestion with the question.
//     const rawSub = r.subquestion ? r.subquestion.trim() : "";
//     const sDisp = normalizeText(rawSub) === normalizeText(qDisp) ? "" : rawSub;
...
// // ──────────────────────────────────────────────
// // Main Component with Row Numbering and Scroll-to-Top on Pagination
// // ──────────────────────────────────────────────
...
//   // Ref for the container wrapping the table, to scroll to top on page change.
...
//   // Scroll to the top of the table container when the page changes.
//   useEffect(() => {
//     if (tableRef.current) {
...
//   // Slice only the current records.
...
//   // Compute unionHeaders from all responses.
//   // If the normalized subquestion equals the normalized question, we set it to empty.
...
//           const rawSub = r.subquestion ? r.subquestion.trim() : "";
//           const sDisp =
//             normalizeText(rawSub) === normalizeText(qDisp) ? "" : rawSub;
//           const key = `${normalizeText(qDisp)}|||${normalizeText(sDisp)}`;
...
//   // Filter headers to include only those with at least one non-empty answer.
...
//               <tr key={survey._id}>
//                 {/* Overall row number is calculated across pages */}
